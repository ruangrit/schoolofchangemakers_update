...
<div>
		<div class="col-xs-8 col-xs-offset-2 self_confirm_center">
            <div  class="col-xs-10" style="background:#FFFFFF; padding-top:30px; padding-bottom:30px">
			<p>ยืนยันด้วยรหัสผ่านทางโทรศัพท์มือถือ</p>
			<p>สามารถทำในภายหลังได้ เพื่อเปลี่ยนสถานะเป็น Verified Member</p>
                <p><a href="/about-verified-member" target="_blank">(คลิกเพื่อดูข้อดีของการเป็น Verified Member)</a></p>
			
			<form action="/changemakers/save_otp" method="POST">
				<?php
				 if($_GET['send-sms'] == 1){
				 	?>
					<p class="alter-wait-otp-for-design">
		            ระบบจะจัดส่ง One-Time-Password ไปยังมือถือที่คุณลงทะเบียนไว้<br>ภายในเวลา 5 นาที กรุณารอสักครู่</p>
				 	<?php
				 }
				 ?>
				 <br/>
				<button type="submit" name="submit" class="btn  self_comfirm_button btn--submit">
					<?php if($_GET['send-sms'] == 1 )
					{ print "กดรับรหัสผ่านอีกครั้ง"; }else{ print "กดรับรหัสผ่าน"; } ?></button>
