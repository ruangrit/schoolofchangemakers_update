...
if(isset($_GET['search'])){
    ...
    else if(empty($news) && count($event) == 2){
        $news_event[0] = $event[0];
        $news_event[1] = $event[1];
    }
    else if(empty($news) && count($event) == 1){
        $news_event[0] = $event[0];
    }
    else if(count($news) == 1  && count($event) == 1){
        $news_event[0] = $news[0];
        $news_event[1] = $event[0];
    }
    else if(count($news) == 1  && count($event) == 2){
        $news_event[0] = $news[0];
        $news_event[1] = $event[0];
    ...
    else if(count($news) == 2 && count($event) > 2){
        $news_event[0] = $news[0];
        $news_event[1] = $news[1];
        $news_event[2] = $event[0];
        $news_event[3] = $event[1];
    ...
    else if(count($news) == 2  && count($event) == 1){
        $news_event[0] = $news[0];
        $news_event[1] = $news[1];
    }
    else if(count($news) == 2 && count($event) == 0){
        $news_event[0] = $news[0];
        $news_event[1] = $news[1];
    }
    else if(count($news) == 0 && count($event) == 2){
        $news_event[0] = $event[0];
        $news_event[1] = $event[1];
    }
    else if(count($news) == 1 && count($event) == 0){
        $news_event[0] = $news[0];
    }
    else if(count($news) == 0 && count($event) == 1){
        $news_event[0] = $event[0];
    }
    ...
    else if(count($news) > 3 &&  count($event) == 0){
        $news_event[0] = $news[0];
        $news_event[1] = $news[1];
        $news_event[2] = $news[2];
        $news_event[3] = $news[3];
...
<div class="margin__top5">
	...
	<div class="col-xs-8 total--right pad0">
        ผลการค้นหา : <span class="hilight"><a href="#knowledge"><?php print count($knowledge); ?> Knowledge</a> </span> | <span class="hilight"><a href="#project"><?php print count($projects); ?> <?php if(count($projects) < 2){print "Project";}else{print "Projects";} ?></a> </span> | <span class="hilight"><a href="#news"><?php print count($news)+count($event); ?> <?php if(count($news)+count($event) < 2){ ?> News&amp;Event <?php }else{  ?> News&amp;Events <?php } ?></a></span> |  <span class="hilight"><a href="#commu"><?php print count($user_community); ?> People</a></span>
    </div>
    ...
    <div class="col-xs-12 margin__top10 knowledge--page">    
        ...
        <div class='search--noresult'>ไม่พบคำค้นหาที่เกี่ยวข้องในหมวด 'Knowledge'</div>
        <?php endif; ?>
        <?php if(count($knowledge)>0) : ?>
		<?php
        $count = 0;
...
$sort = !empty($_GET['sort'])?$_GET['sort']:""; -->
        <?php if(count($knowledge)>4){ ?>
            <div class="viewmore--line row col-xs-12">
                <a href="/search-result?search=<?php echo $link; ?>&type=knowledge" class="viewmore--btn row">VIEW ALL RELATED ARTICLES <i class="glyphicon glyphicon-play-circle"></i>
                </a>
            </div>
        <?php } ?>
        <?php endif; ?>
    ...
    <div class="col-xs-12 margin__top30 project--searchpage" >
        ...
        <div class='search--noresult'>ไม่พบคำค้นหาที่เกี่ยวข้องในหมวด 'Project'</div>
        <?php endif; ?>
        <?php if(count($projects)>0) : ?>
		<?php
    ...
    <div class="col-xs-12 margin__top30">
        ...
        if(count($news_event) == 0 ){
            print "<div class='search--noresult'>ไม่พบคำค้นหาที่เกี่ยวข้องในหมวด 'News&Event'</div>";
        }
        // print_r(count($news));
        // print_r(count($event));
        if(count($news_event) > 0 ){ ?>
            ...
            <div class="col-xs-3 box__pad6 newsevent--page">
                    ...
                    <div id="moredetail__over"  class="field--content boxover">
                       ...
                       <div class="detail__small d__inline-block  link__gray">
                           ...
                           <div class="tag--linelimit__1"><span class=" icon-newspaper txt__gray2"></span> <?php print $value['type']; ?> </div>
            ...
            <div class="viewmore--line row col-xs-12">
                <a href="/search-result?search=<?php echo $link; ?>&type=news-event" class="viewmore--btn row">VIEW ALL RELATED NEWS EVENTS <i class="glyphicon glyphicon-play-circle"></i>
                </a>
            </div>
            <?php } ?>
        <?php } ?>
	...
	<div class="col-xs-12 margin__top30 commu--searchpage">
        ...
        <?php if(count($user_community) == 0) : ?>
            <div class='search--noresult'>ไม่พบคำค้นหาที่เกี่ยวข้องในหมวด 'People'</div>
        <?php endif; ?>
        <?php if(count($user_community)>0) : ?>
		<?php
        $count_user = 0;
        ...
        <?php if(count($user_community) >= 4){ ?>
            <div class="viewmore--line row col-xs-12">
                <a href="/search-result?search=<?php echo $link; ?>&type=user-community" class="viewmore--btn row">VIEW ALL RELATED COMMUNITY <i class="glyphicon glyphicon-play-circle"></i>
                </a>
            </div>
        <?php } ?>
        <?php endif; ?>
